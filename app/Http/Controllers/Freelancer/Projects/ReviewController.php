...
class ReviewController
{
    ...
    {
        $reviews = $request->user()->workreviews()->get();
    ...
    {
        $request->merge(['review_type' => 'freelancer']);

        $request->validate(Review::rules($request->client_id, $request->user()->id, $request->project_id));

        $review = $request->user()->workreviews()->create($request->only(Review::getFillable()));
    ...
    {
        $request->validate(Review::rules($request->client_id, $request->user()->id, $request->project_id, $review->id));
